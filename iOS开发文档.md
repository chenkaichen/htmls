...
> 约束说明：ATT弹窗仅首次同意权限声明时触发；用户无论是否授予ATT权限，均不阻断App核心业务流程。
...
- 配置：跟随App启动流程按需触发ATT授权弹窗，不启动强制弹窗
...
3. App启动流程、权限声明校验、ATT授权分支逻辑完全符合需求
4. 所有SDK集成成功、初始化正常、无报错日志
5. 全设备适配无闪退、无UI、无功能BUG
6. 测试包、正式包可正常构建，满足提审条件
7. 原生与Flutter通信通畅，所有业务回调正常落地
