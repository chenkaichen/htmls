...
3. 登录页
     - 采集：IP + 设备国家码 + [安全环境检测](#安全环境检测) + [appList](#appList)
     - 上报：请求服务器接口获取判定字段A(reviewingA)
4. 登录行为
     - 采集：IP + 设备时区 + 设备版本号 + 设备名 + IDFV + [APP版本号](#review-version-code) + [审核账号](#review-account)
...
<a id="review-version-code"></a>
### 3.3、APP版本号

应用当前版本号：用来判断是否正在审核中，如果当前版本号等于审核版本号，一定进入A面

<a id="review-account"></a>
### 3.4、审核账号
